(function () {
  ...
  async function generate(demo = false) {
    ...
    try {
      ...
      const res = await fetch("/api/generate", {
        ...
        body: JSON.stringify({ brief, languages, count, demo }),
      });
      const data = await res.json();
      ...
      const sourceLabel = demo
        ...
        : isFallback
          ...
          : data.cached
            ? "AI · cached"
            : "AI generated";
      renderDirections(data.directions || [], sourceLabel);
      const statusText = demo
        ? "Demo mode · no Groq call"
        : isFallback
          ? `Local fallback · Groq cooling down${data.retryAfter ? ` (${data.retryAfter}s)` : ""}`
          : data.cached
            ? "AI result · served from cache"
            : "AI result · generated by Groq";
      el("generationStatus").innerHTML = `<span class="status-pip"></span>${statusText}`;
    } catch (err) {
      if (err.name === "AbortError") return;
      errorMsg.textContent = err.message.includes("429")
        ? "Groq has reached its provider rate limit. Try again after the retry time in the error details."
        : err.message || "Something went wrong talking to the model.";
      errorMsg.hidden = false;
      if (err.noKey) setupHint.hidden = false;
      el("generationStatus").innerHTML = '<span class="status-pip status-pip-alert"></span>Generation paused · try demo mode';
    } finally {
  ...
  // ---------- rendering ----------
  function renderDirections(directions, sourceLabel) {
    ...
    directions.forEach((direction, i) => {
      const card = buildCard(direction, { standalone: false, index: i, sourceLabel });
  ...
  function buildCard(direction, opts) {
    ...
    if (!opts.standalone && opts.sourceLabel) {
      const source = document.createElement("span");
      source.className = "card-source";
      source.textContent = opts.sourceLabel;
  ...
  async function regenerateCard(direction, cardEl, btn) {
    ...
    try {
      ...
      const fresh = buildCard(data.direction, { standalone: false, sourceLabel: data.fallback ? "Local fallback" : "AI remix" });
      fresh.style.animationDelay = "0ms";
      cardEl.replaceWith(fresh);
      el("generationStatus").innerHTML = `<span class="status-pip"></span>${data.fallback ? "Local fallback · no Groq call" : "AI remix · generated by Groq"}`;
    } catch (err) {
      btn.textContent = "Retry remix";
      btn.disabled = false;
      btn.title = err.message || "Remix failed. Try again.";
      el("generationStatus").innerHTML = '<span class="status-pip status-pip-alert"></span>Remix paused · try again or use demo mode';
      return;
    }
  }
  ...
  function openDistributeModal(direction) {
    ...
    const link = `${publicBaseUrl}${window.location.pathname}#card=${encoded}`;
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    fetch("/api/config")
      .then((r) => r.json())
      .then((data) => {
        if (data.publicBaseUrl) publicBaseUrl = data.publicBaseUrl;
      })
      .catch(() => {});
    ...
    el("demoBtn").addEventListener("click", () => generate(true));
